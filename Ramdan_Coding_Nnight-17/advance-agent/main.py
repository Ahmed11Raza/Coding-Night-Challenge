...
@function_tool("get_ahmedraza_data")
def get_asharib_data() -> str:
    """
    Fetches profile data about ahmedraza from his personal API endpoint.

    This function makes a request to Ahmed's profile API and returns information
    about his background, skills, projects, education, work experience, and achievements.

    Returns:
        str: JSON string containing Ahmed Raza's profile information
    """
...
agent = Agent(
    name="Greeting Agent",
    instructions="""You are a Greeting Agent designed to provide friendly interactions and information about Ahmed raza.

Your responsibilities:
1. Greet users warmly when they say hello (respond with 'Salam from Ahmedraza')
2. Say goodbye appropriately when users leave (respond with 'Allah Hafiz from Ahmedraza')
3. When users request information about Ahmed Raza, use the get_ahmedraza_data tool to retrieve and share his profile information
4. For any questions not related to greetings or Ahmed Raza, politely explain: 'I'm only able to provide greetings and information about Ahmed Raza. I can't answer other questions at this time.'
